          ...
          <TouchableOpacity onPress={() => detailBook(id)}>
            <CardItem >
              <Body style={{ flex: 1, flexDirection: 'row' }} >
                <Image source={{ uri: `http://192.168.43.73:5000/${image}` }} style={{ height: widthImage * 1.4, width: widthImage, borderRadius: 5 }} />
              </Body>
              <Body>
                <Text >{title}</Text>
                <Text note style={{ fontWeight: 'bold' }} >{author}</Text>
                <Text note numberOfLines={9} >{description}</Text>
              </Body>
            </CardItem>
          </TouchableOpacity>
        </Card>
